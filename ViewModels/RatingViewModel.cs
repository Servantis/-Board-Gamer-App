...
{
    public ObservableCollection<RatingStar> RatingItems { get; set; }
    ...
    {
        RatingItems = new ObservableCollection<RatingStar>();

        for (int i = 1; i <= 5; i++)
        {
            int starValue = i;

            RatingItems.Add(new RatingStar
            {
                Image = "star_empty.png",
                //TapCommand = new Command(() => Rating = starValue)
